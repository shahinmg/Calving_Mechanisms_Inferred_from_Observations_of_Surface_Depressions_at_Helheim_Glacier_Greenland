...
i = 0
for block_idx, block_row in flexure_blocks.iterrows():
    ...
    exx_pkl = exx_pkl.iloc[:,:-2]
    exx_sample_pkl = exx_pkl.loc[:,min_date:max_date] # velocity
    exx_dates = exx_sample_pkl.loc[:,~exx_sample_pkl.T.duplicated(keep='first')]
    
    exx_term_sample = []
    for col in exx_dates:
        if col in term_gdf.index:
            term_pt_vel = term_gdf.loc[col,:]
            x_pt_exx = list(term_pt_vel.geometry.coords)[0][0]
            # print(x_pt_vel)
            
            term_idx_vel = x_atc[x_atc == x_pt_exx].index[0]
            exx_sample = exx_dates.loc[term_idx_vel+300:term_idx_vel+900,col].mean()
            exx_term_sample.append(exx_sample)
        else: 
          exx_term_sample.append(np.nan)    
    
    days_from_calving = pd.Series(exx_dates.columns + pd.Timedelta(6,'H')) - mid_date
    
    
    date_from_ax2 = mdates.DateFormatter('%m-%d')
    # print(vel_term_sample)
    percent_change = (exx_term_sample - np.nanmean(exx_term_sample))*100
    exx_sample_data_days = days_from_calving + pd.Timedelta(6,'h') # added 12 to get mid date for 24 hour vels #/day 
    ...
    i+=1
    days_from_calving_df = pd.DataFrame(exx_sample_data_days)
    days_from_calving_df[date1] = exx_term_sample
...
pkl_exx = '/media/laserglaciers/upernavik/flexure_manuscipt/pkls/'



df_all.rename(columns={0:'days_around_calving'},inplace=True)
df_all['days'] = df_all['days_around_calving'].dt.days
df_all.to_pickle(f'{pkl_exx}exx_flexure_style_calving.pkl')
#%%
